...
class SoundController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if ($project->company_id == Yii::app()->user->company && (in_array('admin', Yii::app()->user->roles) || in_array(Yii::app()->user->id, explode(',', $project->team) || Yii::app()->user->id == $project->leader_id))){
			...
			$users = CHtml::listData(
				User::model()->findAllbyAttributes(array(), 'roles LIKE \'%composer%\' AND company_id = ' . Yii::app()->user->company),
				'id',
				'name'
			);
			...
			if(isset($_POST['Sound']))
			{
				...
				if($model->save())
					$this->redirect(array('project/sounds','id'=>$project->id));
			}
	...
	public function loadModel()
	{
		...
		{
			if(isset($_GET['id']))
				$this->_model=Sound::model()->findbyPk($_GET['id']);
			if($this->_model===null)
