...
<section class="content-header">
  <h1 class="hidden-xs"><?php echo Language::$development; ?> <small><?php echo Language::$level; ?></small></h1>
...
<div class="jumbotron" data-step="1" data-intro="<?php echo Language::$developmentTaskHelp1; ?>" data-position="bottom">
  <div class="container">
      ...
      <?php if (in_array('admin', Yii::app()->user->roles) or in_array('designer', Yii::app()->user->roles) and (!$model->finish or ($model->code and !$model->valid)) ){ $activities = true; ?>
          ...
          <?php if (!$model->finish){ ?>
            <h1><?php echo Language::$viewDesign ?></h1>
          ...
          <?php } else if (!$model->valid){ ?>
            ...
            <p><?php echo Language::$registerWorktime; ?></p>
  					...
  					<div class="form-group buttons">
  					  <?php echo CHtml::submitButton(Language::$markAsValid, array('class'=>'btn btn-primary btn-lg', 'onclick'=>'submitTime()')); ?>
  				  </div>
          <?php } ?>
		    <?php $this->endWidget(); ?>
			<?php } ?>
			
      <?php if (in_array('admin', Yii::app()->user->roles) or in_array('programmer', Yii::app()->user->roles) and $model->finish and !$model->code){ $activities = true; ?>
  			<?php $form=$this->beginWidget('CActiveForm', array(
					'id'=>'level-form',
					'enableAjaxValidation'=>false,
					'htmlOptions'=>array('enctype'=>'multipart/form-data'),
        ...
        <p><?php echo Language::$registerWorktime; ?></p>
				...
				<div class="form-group buttons">
				  <?php echo CHtml::submitButton(Language::$markAsCoded, array('class'=>'btn btn-warning btn-lg', 'onclick'=>'submitTime()')); ?>
			  </div>
	      <?php $this->endWidget(); ?>
			<?php } ?>
...
<div class="row">
            ...
            <div class="box-header with-border">
              ...
              <div class="box-body">
                  ...
                  <select id="state" name="state" class="form-control">
                    <option value="finish" <?php if (!$model->finish){ echo "selected"; } ?>><?php echo Language::$viewDesign; ?></option>
                    <option value="code" <?php if ($model->finish and !$model->code){ echo "selected"; } ?>><?php echo Language::$viewCode; ?></option>
    ...
    <?php if (count($changes) > 0){ foreach($changes as $c){ ?>
      ...
      <div class="box box-success box-solid">
          ...
          <div class="box-body">
                  ...
                  echo '<h4>' . Language::$synopsis . '</h4>';
                  echo '<p>' . $model->synopsis . '</p>';
                }
    					  if($model->introduction != null){
                  echo '<h4>' . Language::$introduction . '</h4>';
                  ...
                  echo '<h4>' . Language::$gameObjectives . '</h4>';
                  echo '<p>' . $model->objectives . '</p>';
                }
    					  if($model->encounters != null){
                  echo '<h4>' . Language::$encounters . '</h4>';
                  echo '<p>' . $model->encounters . '</p>';
                }
    					  if($model->path != null){
                  echo '<h4>' . Language::$path . '</h4>';
                  echo '<p>' . $model->path . '</p>';
                }
    					  if($model->walkthrough != null){
                  echo '<h4>' . Language::$walkthrough . '</h4>';
                  echo '<p>' . $model->walkthrough . '</p>';
                }
    					  if($model->closing != null){
                  echo '<h4>' . Language::$closing . '</h4>';
                  echo '<p>' . $model->closing . '</p>';
                }
    					?>
  	    </div>
      		<?php
      			if($model->design != null){
        				...
        				$file = Yii::app()->request->baseUrl . '/files/' . Yii::app()->user->company . '/' . $project->id . '/levels/' . $model->id . '.' . end($ext);
      			    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $file)){
      				    echo '<div class="box-footer text-center">';
      				    echo '<h4>' . Language::$design . '</h4>';
      				    echo '<p>
      				          <img src="' .  $file . '" class="img-responsive"/><br/>
      				          <a href="' .  $file . '" class="btn btn-info"><span class="glyphicon glyphicon-circle-arrow-down"></span> ' . Language::$download . '</a></p>';
      				    echo '</div>';
      				}
      			}
      		?>
    ...
    <section class="col-lg-12 connectedSortable ui-sortable">
        <div class="box box-success">
            ...
            <div class="box-body">
              <?php
                if ($model->time_est){
                    echo '<p>' . sprintf(Language::$levelTimeMessage, Functions::getTime($model->time_est * 60)) . '</p>';
                }
  ...
  <?php if (count($graphics) > 0 or count($sounds) > 0 or count($musics) > 0 or count($characters) > 0 or count($enviroments) > 0 or count($cutscenes) > 0){ ?>
  <div class="nav-tabs-custom">
    <ul class="nav nav-tabs">
      ...
      <?php } ?>
    	<?php if (count($enviroments) > 0){ ?>
      	<li><a href="#tab_enviroment" data-toggle="tab"><?php echo Language::$enviromentObjects; ?></a></li>
      <?php } ?>
    	<?php if (count($cutscenes) > 0){ ?>
      	<li><a href="#tab_cutscenes" data-toggle="tab"><?php echo Language::$cutscenes; ?></a></li>
      <?php } ?>
    </ul>
    ...
    <div class="tab-content">
      ...
      <div class="tab-pane" id="tab_enviroment">
  			<?php
  				foreach($enviroments as $enviroment){
    			  echo '<p>' . $enviroment->name . '</p>';
  				}
  			?>
      </div>
      
      <div class="tab-pane" id="tab_cutscenes">
  			<?php
  				foreach($cutscenes as $cutscene){
    			  echo '<p>' . $cutscene->description . '</p>';
  				}
  			?>
      </div>
    </div>
