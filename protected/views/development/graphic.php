...
<section class="content-header">
  <h1 class="hidden-xs"><?php echo Language::$development; ?> <small><?php echo Language::$graphic; ?></small></h1>
...
<div class="jumbotron" data-step="1" data-intro="<?php echo Language::$developmentTaskHelp1; ?>" data-position="bottom">
  <div class="container">
      ...
      <?php if (in_array('admin', Yii::app()->user->roles) or in_array('artist', Yii::app()->user->roles) and (!$model->sketch or ($model->valid and !$model->finish))){ $activities = true; ?>
          ...
          <?php } else if (!$model->finish){ ?>
            <h1><?php echo Language::$viewFinal ?></h1>
            <p><?php echo Language::$registerWorktime; ?></p>
  					<div class="form-group">
  						<?php echo $form->fileField($model,'file'); ?>
  					...
  					<div class="form-group buttons">
  					  <?php echo CHtml::submitButton(Language::$update, array('class'=>'btn btn-info btn-lg', 'onclick'=>'submitTime()')); ?>
  				  </div>
          <?php } ?>
		    <?php $this->endWidget(); ?>
			<?php } ?>
            ...
            <div class="box-header with-border">
              ...
              <div class="box-body">
                  ...
                  <select id="state" name="state" class="form-control">
                    <option value="sketch" <?php if (!$model->sketch){ echo "selected"; } ?>><?php echo Language::$viewSketch; ?></option>
                    <option value="valid" <?php if ($model->sketch and !$model->valid){ echo "selected"; } ?>><?php echo Language::$viewValid; ?></option>
                    <option value="finish" <?php if ($model->valid and !$model->finish){ echo "selected"; } ?>><?php echo Language::$viewFinal; ?></option>
                  </select>
              	</div>
              	<div class="form-group">
    ...
    <section class="col-lg-12 connectedSortable ui-sortable">
        <div class="box box-success">
            ...
            <div class="box-body">
              <?php
                if ($model->time_est){
                    echo '<p>' . sprintf(Language::$graphicTimeMessage, Functions::getTime($model->time_est * 60)) . '</p>';
                }
                if (count($timesheets) > 0){
                    echo '<p>' . sprintf(Language::$graphicTimeCurrentMessage, Functions::getTime($model->time)) . '</p>';
                    //print part times
