...
<section class="content-header">
  <h1 class="hidden-xs"><?php echo Language::$development; ?> <small><?php echo Language::$cutscene; ?></small></h1>
  ...
  <ol class="breadcrumb">
    ...
    <li class="active"><?php echo $model->description; ?></li>
  </ol>
</section>
...
<div class="jumbotron" data-step="1" data-intro="<?php echo Language::$developmentTaskHelp1; ?>" data-position="bottom">
    <div class="container">
        ...
        <?php if (in_array('admin', Yii::app()->user->roles) or in_array('writer', Yii::app()->user->roles) and !$model->finish){ $activities = true; $submit = true; ?>
            <?php $form=$this->beginWidget('CActiveForm', array(
                'id'=>'scene-form',
                'enableAjaxValidation'=>false,
                'htmlOptions'=>array('enctype'=>'multipart/form-data'),
                )); ?>
                <h1><?php echo Language::$viewWrite ?></h1>
                <p><?php echo Language::$registerWorktime; ?></p>
                <div class="form-group">
                    <h2><?php echo Language::$script; ?></h2>
                    <?php echo $form->textArea($model,'script', array('class'=>'form-control textarea', 'placeholder'=>Language::$scriptDesc)); ?>
                </div>
                <div class="form-group">
               		<label>
    			        <?php echo $form->checkBox($model,'finish', array('checked'=>'0')); ?>
    			        <?php echo Language::$markAsDone; ?>
			        </label>
                </div>
                <div class="form-group buttons">
                    <?php echo CHtml::submitButton(Language::$save, array('class'=>'btn btn-danger btn-lg', 'onclick'=>'submitTime()')); ?>
                </div>
            <?php $this->endWidget(); ?>
        <?php } ?>

        <?php if (in_array('admin', Yii::app()->user->roles) or in_array('artist', Yii::app()->user->roles) and $model->finish and !$model->sketch){ $activities = true; ?>
            <?php $form=$this->beginWidget('CActiveForm', array(
                'id'=>'scene-form',
                'enableAjaxValidation'=>false,
                'htmlOptions'=>array('enctype'=>'multipart/form-data'),
                )); ?>
                <h1><?php echo Language::$viewSketch ?></h1>
                <p><?php echo Language::$registerWorktime; ?></p>
                <div class="form-group">
               		<label>
                        <?php echo $form->fileField($model,'storyboard'); ?>
                        <?php echo $form->checkBox($model,'sketch', array('checked'=>'1', 'hidden'=>'1')); ?>
			        </label>
                </div>
                <div class="form-group buttons">
                    <?php echo CHtml::submitButton(Language::$save, array('class'=>'btn btn-warning btn-lg', 'onclick'=>'submitTime()')); ?>
                </div>
            <?php $this->endWidget(); ?>
        <?php } ?>

        <?php if (in_array('admin', Yii::app()->user->roles) or in_array('designer', Yii::app()->user->roles) and $model->sketch and !$model->valid){ $activities = true; ?>
            <?php $form=$this->beginWidget('CActiveForm', array(
                'id'=>'scene-form',
                'enableAjaxValidation'=>false,
                'htmlOptions'=>array('enctype'=>'multipart/form-data'),
                ...
                <div class="form-group buttons">
                    <?php echo CHtml::submitButton(Language::$markAsValid, array('class'=>'btn btn-default btn-lg', 'onclick'=>'submitTime()')); ?>
                </div>
            <?php $this->endWidget(); ?>
        <?php } ?>

        <?php if (in_array('admin', Yii::app()->user->roles) or in_array('programmer', Yii::app()->user->roles) and $model->valid and !$model->code){ $activities = true; ?>
            <?php $form=$this->beginWidget('CActiveForm', array(
            'id'=>'scene-form',
            'enableAjaxValidation'=>false,
            'htmlOptions'=>array('enctype'=>'multipart/form-data'),
                ...
                <div class="form-group buttons">
                    <?php echo CHtml::submitButton(Language::$markAsCoded, array('class'=>'btn btn-primary btn-lg', 'onclick'=>'submitTime()')); ?>
                </div>
            <?php $this->endWidget(); ?>
        <?php } ?>
...
<section class="content" data-step="3" data-intro="<?php echo Language::$developmentTaskHelp3; ?>" data-position="top">
            ...
            <div class="box-header with-border">
              ...
              <div class="box-body">
                  ...
                  <select id="state" name="state" class="form-control">
                    <option value="finish" <?php if (!$model->finish){ echo "selected"; } ?>><?php echo Language::$viewWrite; ?></option>
                    <option value="sketch" <?php if ($model->finish and !$model->sketch){ echo "selected"; } ?>><?php echo Language::$viewSketch; ?></option>
                    <option value="valid" <?php if ($model->sketch and !$model->valid){ echo "selected"; } ?>><?php echo Language::$viewValid; ?></option>
                    <option value="code" <?php if ($model->valid and !$model->code){ echo "selected"; } ?>><?php echo Language::$viewCode; ?></option>
    ...
    <?php if (count($changes) > 0){ foreach($changes as $c){ ?>
        ...
        <section class="col-lg-12 connectedSortable ui-sortable">
            <div class="box box-success box-solid">
                ...
                <div class="box-body">
                    <?php
						if($model->script != null){
						    echo '<h4>' . Language::$script . '</h4>';
						    echo '<p>' . $model->script . '</p>';
						}
                    ?>
                </div>
                <?php
                if($model->storyboard != null){
      				...
      				$file = Yii::app()->request->baseUrl . '/files/' . Yii::app()->user->company . '/' . $project->id . '/graphics/' . $model->id . '.' . end($ext);
      				if (file_exists($_SERVER['DOCUMENT_ROOT'] . $file)){
    				    echo '<div class="box-footer text-center">';
    				    echo '<h4>' . Language::$storyboard . '</h4>';
    				    echo '<p>
    				          <img src="' . $file . '" class="img-responsive center-block"/><br/>
    				          <a href="' . $file . '" class="btn btn-info" download><span class="glyphicon glyphicon-circle-arrow-down"></span> ' . Language::$download . '</a></p>';
    				    echo '</div>';
        ...
        <section class="col-lg-12 connectedSortable ui-sortable">
            <div class="box box-success">
                ...
                <div class="box-body">
                  <?php
                    if ($model->time_est){
                        echo '<p>' . sprintf(Language::$cutsceneTimeMessage, Functions::getTime($model->time_est * 60)) . '</p>';
                    }
                    if (count($timesheets) > 0){
                        echo '<p>' . sprintf(Language::$cutsceneTimeCurrentMessage, Functions::getTime($model->time)) . '</p>';
                        //print part times
        ...
        <?php if($project->style != null){ ?>
            <section class="col-lg-6 connectedSortable ui-sortable">
                <div class="box box-success">
                    <div class="box-header with-border">
                        <h3 class="box-title"><?php echo Language::$artStyle; ?></h3>
                        <div class="box-tools pull-right">
                            <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
                            <button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-times"></i></button>
                        </div>
    ...
    <?php if (count($graphics) > 0 or count($sounds) > 0 or count($musics) > 0 or count($characters) > 0 or count($enviroments) > 0){ ?>
        <div class="box nav-tabs-custom">
            <ul class="nav nav-tabs">
                ...
                <?php if (count($musics) > 0){ ?>
                    <li><a href="#tab_music" data-toggle="tab"><?php echo Language::$music; ?></a></li>
                <?php } ?>
                <?php if (count($characters) > 0){ ?>
                    <li><a href="#tab_characters" data-toggle="tab"><?php echo Language::$characters; ?></a></li>
                <?php } ?>
                <?php if (count($enviroments) > 0){ ?>
                    <li><a href="#tab_enviroments" data-toggle="tab"><?php echo Language::$enviromentObjects; ?></a></li>
                <?php } ?>
            </ul>
            <div class="tab-content">
                ...
                <div class="tab-pane" id="tab_music">
                    ...
                    foreach($musics as $music){
                        ...
                        $file = Yii::app()->request->baseUrl . '/files/' . Yii::app()->user->company . '/' . $project->id . '/music/' . $music->id . '.' . end($ext);
                        if (file_exists($_SERVER['DOCUMENT_ROOT'] . $file)){
                ...
                <div class="tab-pane" id="tab_enviroments">
                    <?php
                    foreach($enviroments as $enviroment){
                        echo '<h4>' . $enviroment->name . '</h4>';
                    }
                    ?>
                </div>
            </div>
